...
public class baseGUI extends JFrame implements ActionListener {
  ...
  public baseGUI(Game game) {
    ...
    jlDifferentVersion = new JLabel(
        "<html>This mod was made for<br> different game version, it<br> might not work</html>");
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() == jbInstall) {
      Mod.installMod(new File(Game.getGamePath() + "\\" + selectedMod.getInstallTo()),
          new File(Game.getGamePath() + "\\mods\\" + selectedMod.getName() + ".mod"));
  ...
  private void changeSelected() {
    ...
    String selected =
        jcbModSelector.getSelectedItem() != null ? jcbModSelector.getSelectedItem().toString()
            : null;
    if (selected == null) {
      return;
    }
    ...
    if (!selectedMod.getIntendedVersion().equals(game.getGameVersion()) && !jlDifferentVersion
        .isShowing()) {
